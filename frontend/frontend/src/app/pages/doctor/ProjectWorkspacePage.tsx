...
import { recommendStudents } from "../../../api/ai";
...
export default function ProjectWorkspacePage() {
  ...
  const [aiStudents, setAiStudents] = useState<
    { studentId: number; matchScore: number; reason?: string | null }[]
  >([]);
  ...
  const handleFindTeammates = async () => {
    ...
    try {
      setAiLoading(true);
      const data = await recommendStudents(project.id);
      setAiStudents(Array.isArray(data) ? data : []);
    } catch (err) {
      console.error(err);
    } finally {
      setAiLoading(false);
  ...
  return (
    <div style={S.page}>
      ...
      <div style={S.content}>
        ...
        <div style={S.header}>
              ...
              <div style={S.metaRow}>
                {project.maxTeamSize && (
                  <span style={S.metaItem}>
                    <Users size={13} /> Up to {project.maxTeamSize} students
                  </span>
                )}
                {project.dueDate && (
                  <>
          ...
          <div style={S.statsRow}>
            <StatPill
              icon={<Sparkles size={12} color="#6366f1" />}
              value={aiStudents.length}
              label="AI Matches"
              color="#6366f1"
            />
            <StatPill
              icon={<Users size={12} color="#10b981" />}
              value={teamMembers.filter((m) => m.status === "accepted").length}
              label="Accepted"
              color="#10b981"
            />
            <StatPill
              icon={
                <CheckCircle2
                  size={12}
                  color={supervisor ? "#10b981" : "#94a3b8"}
                />
              }
              value={supervisor ? 1 : 0}
              label="Supervisor"
              color={supervisor ? "#10b981" : "#94a3b8"}
            />
          </div>
          ...
          <div style={{ display: "flex", flexDirection: "column", gap: 16 }}>
            ...
            <InfoSection title="Team Configuration">
              <div style={{ display: "flex", gap: 32, flexWrap: "wrap" }}>
                <div>
                  ...
                  <p style={S.infoValue}>
                    {project.maxTeamSize ?? "—"} students
                  </p>
                </div>
                <div>
                  <span style={S.infoLabel}>Formation Mode</span>
                  <p style={S.infoValue}>
                    {project.formationMode === "doctor"
                      ? "AI / Supervisor"
                      : "Student Choice"}
                  </p>
                </div>
            ...
            >
              <button
                ...
                disabled={aiLoading}
              >
                <Sparkles size={13} /> Find Teammates (AI)
              </button>
              <button
                style={S.primaryBtn}
          ...
          <div>
            <div style={S.sectionHeader}>
              <div>
                ...
                >
                  ...
                  <span style={S.sectionCount}>{aiStudents.length}</span>
                </div>
            ...
            {aiStudents.length === 0 && !aiLoading ? (
              <div style={S.empty}>
                ...
                <p
                  style={{ color: "#cbd5e1", margin: "0 0 14px", fontSize: 12 }}
                >
                  Click &quot;Find Teammates (AI)&quot; to load recommendations
                  from the server.
                </p>
                <button
                  type="button"
              ...
              <div style={S.cardList}>
                {aiStudents.map((s) => (
                  <div key={s.studentId} className="ai-card">
                    <p>
                      <strong>ID:</strong> {s.studentId}
                    </p>
                    <p>
                      <strong>Match:</strong> {s.matchScore}
                    </p>
                    <p>
                      <strong>Reason:</strong> {s.reason || "No reason"}
                    </p>
            ...
            {teamMembers.length === 0 ? (
              <div style={S.empty}>
                ...
                <p
                  style={{ color: "#cbd5e1", margin: "0 0 14px", fontSize: 12 }}
                >
                  Invite students from AI suggestions or send a direct
                  invitation.
                </p>
                <button
                  style={S.primaryBtn}
              ...
              <div style={S.cardList}>
                {teamMembers.map((m) => (
                  <div key={m.id} style={S.suggestionCard}>
                    ...
                    <span
                      style={m.status === "accepted" ? S.tagGreen : S.tagAmber}
                    >
            ...
            {!supervisor ? (
              <div style={S.empty}>
                ...
                <p
                  style={{ color: "#cbd5e1", margin: "0 0 14px", fontSize: 12 }}
                >
                  The AI will suggest the most suitable supervisor based on this
                  project's topic and required expertise.
                </p>
              ...
              <div style={S.suggestionCard}>
                ...
                >
                  <div
                    style={{ ...S.avatar, width: 44, height: 44, fontSize: 14 }}
                  >
                ...
                >
                  ...
                  {supervisor.status === "suggested" && (
                    <button style={S.primaryBtn}>Send Request</button>
                  )}
                  {supervisor.status === "requested" && (
      ...
      <style>{`
                ...
                .ai-card p { margin: 6px 0; font-size: 13px; color: #0f172a; }
                .ai-card button {
                    margin-top: 8px;
                    padding: 6px 12px;
                    border-radius: 8px;
