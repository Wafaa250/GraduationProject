...
export function FeedStudentPostComposer({ onPosted }: Props) {
  ...
  const handleImageChange = (e: ChangeEvent<HTMLInputElement>) => {
    ...
    if (!isStudentPostImageFile(file)) {
      toast({
        variant: "destructive",
        title: "Invalid image",
  ...
  const handleFileChange = (e: ChangeEvent<HTMLInputElement>) => {
    ...
    if (!isStudentPostDocumentFile(file)) {
      toast({
        variant: "destructive",
        title: "Invalid file",
  ...
  return (
    <section className="hub-card feed-composer" aria-label="Create a post">
      <Textarea
        ref={textareaRef}
        value={content}
        ...
        rows={3}
        className="feed-composer__input min-h-[5.5rem] resize-y border-0 bg-transparent p-0 shadow-none focus-visible:ring-0"
        disabled={posting}
      />
