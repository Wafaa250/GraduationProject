  ...
  return {
    ...
    description:
      ...
      buildCourseDescription(
        { name: enrolled.name, code: enrolled.code, semester: enrolled.semester },
      ),
...
export function mapDetailToModel(
  ...
  return {
    ...
    schedule: formatSectionSchedule(
      detail.sections.find((s) => s.id === detail.mySectionId),
    ),
    officeHours,
  };
...
/** Derives per-classmate team labels from my-team and student-led roster endpoints. */
