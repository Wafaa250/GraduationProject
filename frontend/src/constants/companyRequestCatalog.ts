...
/** Project focus areas — all disciplines, not tech-only. */
...
/** Roles from every student discipline pool, plus common cross-sector titles. */
...
const ALL_POOL_SKILLS = [
  ...new Set(
    Object.values(SKILLS_DATA).flatMap((p) => [...p.technicalSkills, ...p.tools]),
  ),
];
...
/** Searchable roles — university-wide, all faculties. */
export const COMPANY_ROLE_OPTIONS = [
  ...new Set([...ALL_ROLES, ...CROSS_SECTOR_ROLES]),
].sort((a, b) => a.localeCompare(b));

/** Searchable skills — technical, tools, and cross-disciplinary competencies. */
...
export function collaborationFormatLabel(value: string): string {
  return COLLABORATION_FORMATS.find((f) => f.value === value)?.label ?? value;
}
