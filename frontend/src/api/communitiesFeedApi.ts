...
export function deriveRegistrationStatus(detail: PublicOrganizationEventDetail): EventRegistrationStatus {
...
async function fetchOrgFeedSlice(org: PublicOrganizationDiscovery): Promise<OrgFeedSlice> {
  try {
...
async function enrichEventRegistration(event: CommunityFeedEvent): Promise<CommunityFeedEvent> {
  try {
    ...
    return {
      ...event,
      registrationStatus: deriveRegistrationStatus(detail),
    }
  } catch {
    return event
  }
...
function buildActivity(
  ...
  for (const event of events) {
    ...
    items.push({
      ...
      link: `/organizations/${event.organizationId}/events/${event.id}`,
    })
  }
  ...
  for (const campaign of recruitment) {
    ...
    items.push({
      ...
      link: `/organizations/${campaign.organizationId}/recruitment-campaigns/${campaign.id}`,
    })
  }
...
export async function loadCommunityFeed(
  ...
  const activity =
    followedIds.size > 0
      ? buildActivity(events, recruitment, followedIds)
      : []
