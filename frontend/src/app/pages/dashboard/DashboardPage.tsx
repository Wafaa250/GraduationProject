...
export default function DashboardPage() {
  ...
  const handleGradSubmit = () => {
    ...
    setGradProject({
      ...
      skills:      gradForm.skills.split(',').map((s: string) => s.trim()).filter(Boolean),
      teamSize:    size,
    })
                ...
                <div style={{ padding: '14px', background: 'linear-gradient(135deg,rgba(99,102,241,0.05),rgba(168,85,247,0.05))', border: '1px solid rgba(99,102,241,0.15)', borderRadius: 12 }}>
                  ...
                  {gradProject.skills.length > 0 && (
                    <div style={{ display: 'flex', flexWrap: 'wrap' as const, gap: 4, marginBottom: 10 }}>
                      {gradProject.skills.map((sk: string) => <span key={sk} style={S.skillChipSm}>{sk}</span>)}
                    </div>
                  )}
                    ...
                    <div style={{ marginTop: 12, borderTop: '1px solid rgba(99,102,241,0.12)', paddingTop: 12 }}>
                      ...
                      <div style={{ display: 'flex', flexDirection: 'column' as const, gap: 7 }}>
                        ...
                        <div style={{ display: 'flex', alignItems: 'center', gap: 10, padding: '9px 11px', background: 'white', border: '1px solid rgba(99,102,241,0.18)', borderRadius: 10 }}>
                          ...
                          {allSkills.slice(0, 2).map((sk: string) => (
                        ...
                        {gradTeammates.map(t => (
                          <div key={t.userId} style={{ display: 'flex', alignItems: 'center', gap: 10, padding: '9px 11px', background: '#f8fafc', border: '1px solid #e2e8f0', borderRadius: 10 }}>
                            ...
                            <div style={{ display: 'flex', gap: 4, flexWrap: 'wrap' as const }}>
                              {t.skills.slice(0, 2).map((sk: string) => (
                                <span key={sk} style={S.skillChipSm}>{sk}</span>
                              ))}
                            </div>
                            <button
                              onClick={() => setGradTeammates(prev => prev.filter(x => x.userId !== t.userId))}
                              style={{ background: 'none', border: 'none', cursor: 'pointer', color: '#cbd5e1', fontSize: 13, padding: 0, lineHeight: 1, fontFamily: 'inherit', flexShrink: 0 }}>✕</button>
                          </div>
                        ))}
                      </div>
                    </div>
          ...
          <div style={S.modalBox} onClick={e => e.stopPropagation()}>
            ...
            {[
              ...
              <div key={key} style={{ marginBottom: 14 }}>
                ...
                <input type={type} style={S.modalInput}
            ...
            <div style={{ display: 'flex', justifyContent: 'flex-end', gap: 10, marginTop: 22 }}>
              ...
              <button onClick={handleSaveInfo} disabled={editSaving}
                style={{ padding: '9px 22px', borderRadius: 10, border: 'none', background: editSuccess ? '#16a34a' : 'linear-gradient(135deg,#6366f1,#a855f7)', color: 'white', fontSize: 13, fontWeight: 700, cursor: editSaving ? 'not-allowed' : 'pointer', fontFamily: 'inherit', minWidth: 120 }}>
        ...
        <div style={S.modalOverlay} onClick={() => { setGradModalOpen(false); setGradFormError(null) }}>
          <div style={S.modalBox} onClick={e => e.stopPropagation()}>
            ...
            <div style={{ display: 'flex', justifyContent: 'flex-end', gap: 10 }}>
              ...
              <button onClick={handleGradSubmit} style={{ padding: '9px 22px', borderRadius: 10, border: 'none', background: 'linear-gradient(135deg,#6366f1,#a855f7)', color: 'white', fontSize: 13, fontWeight: 700, cursor: 'pointer', fontFamily: 'inherit' }}>Create Project</button>
            </div>
          </div>
        </div>
      )}
          ...
          <div style={{ ...S.modalBox, width: 520, maxHeight: '80vh', overflowY: 'auto' as const }} onClick={e => e.stopPropagation()}>
            <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 18 }}>
              <div>
                ...
                <p style={{ margin: 0, fontSize: 12, color: '#94a3b8' }}>{gradTeammates.length} / {gradProject?.teamSize ?? '—'} partners selected</p>
              </div>
                  ...
                  return (
                    <div key={t.userId} style={{ display: 'flex', alignItems: 'flex-start', gap: 12, padding: '12px 14px', background: isInvited ? '#f5f3ff' : '#f8fafc', border: `1px solid ${isInvited ? '#c7d2fe' : '#e2e8f0'}`, borderRadius: 12 }}>
                      ...
                      <div style={{ flex: 1, minWidth: 0 }}>
                        ...
                        <div style={{ display: 'flex', flexWrap: 'wrap' as const, gap: 4 }}>
                          {t.skills.slice(0, 3).map((sk: string) => <span key={sk} style={S.skillChipSm}>{sk}</span>)}
                        </div>
                      </div>
                      {isInvited ? (
                        <button onClick={() => setGradTeammates(prev => prev.filter(x => x.userId !== t.userId))}
                        ...
                        <button disabled={isFull} onClick={() => setGradTeammates(prev => [...prev, t])}
            ...
            <div style={{ display: 'flex', justifyContent: 'flex-end', marginTop: 18 }}>
              <button onClick={() => setAddTeammatesOpen(false)}
