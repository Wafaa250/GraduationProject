...
import { LogOut, Settings, Briefcase, ClipboardList, Users } from 'lucide-react'
...
interface GradProjectListItem {
    ...
    isFull: boolean
    supervisor?: { doctorId: number; name: string; specialization?: string | null } | null
}
...
export default function DoctorDashboardPage() {
    ...
    const [supervisedProjects, setSupervisedProjects] = useState<GradProjectListItem[]>([])
    ...
    const loadSupervisedProjects = useCallback(async (doctorProfileId: number) => {
        setProjectsLoading(true)
        setProjectsError(null)
        try {
            const { data } = await api.get<GradProjectListItem[]>('/graduation-projects')
            const list = Array.isArray(data) ? data : []
            setSupervisedProjects(
                list.filter(
                    (p) => p.supervisor != null && p.supervisor.doctorId === doctorProfileId,
                ),
            )
        } catch (err: unknown) {
    ...
    useEffect(() => {
        const run = async () => {
            ...
            try {
                ...
                setMe(data)
                await Promise.all([loadRequests(), loadSupervisedProjects(data.profileId)])
            } catch (err: unknown) {
            ...
            <div style={S.page}>
                ...
                <div style={{ ...S.content, maxWidth: 560 }}>
                    <div style={S.card}>
                        ...
                        <button type="button" onClick={() => navigate('/login')} style={{ ...S.heroBtn, marginTop: 14 }}>
                            Back to login
                        </button>
                    </div>
                </div>
            ...
            <nav style={S.nav}>
                <div style={S.navInner}>
                    ...
                    <div style={S.navActions}>
                        ...
                        <button type="button" style={S.navBtn} onClick={() => { localStorage.clear(); navigate('/login') }}>
                            <LogOut size={16} />
                        </button>
            ...
            <div style={S.content}>
                <div style={S.hero}>
                    <div style={S.heroLeft}>
                        ...
                        <p style={S.heroSub}>Manage your supervised projects and requests</p>
                        <p style={S.heroMeta}>
                            {me.name}
                    ...
                    <div ref={requestsRef} id="doctor-requests" style={S.card}>
                        ...
                        {requestsLoading ? (
                            ...
                            <div style={S.emptyState}>
                                ...
                                <p style={S.emptyDesc}>Supervision and cancellation requests will appear here.</p>
                            </div>
                        ) : (
                                                ...
                                                <div style={{ minWidth: 0, flex: '1 1 200px' }}>
                                                    ...
                                                    <span
                                                        style={{
                                                            ...
                                                            color: row.kind === 'cancellation' ? '#b45309' : '#6366f1',
                                                        }}
                                                    >
                                                            ...
                                                            <button
                                                                ...
                                                                onClick={() =>
                                                                    row.kind === 'supervision'
                                                                        ? handleSupervisionAction(row.requestId, 'accept')
                                                                        : handleCancelAction(row.requestId, 'accept')
                                                                }
                                                                style={S.acceptBtn}
                                                            >
                                                                {actionKey === `${row.kind === 'supervision' ? 'sup' : 'can'}-${row.requestId}-accept`
                                                                    ? '…'
                                                                    : 'Accept'}
                                                            </button>
                                                            <button
                                                                ...
                                                                onClick={() =>
                                                                    row.kind === 'supervision'
                                                                        ? handleSupervisionAction(row.requestId, 'reject')
                                                                        : handleCancelAction(row.requestId, 'reject')
                                                                }
                                                                style={S.rejectBtn}
                                                            >
                                                                {actionKey === `${row.kind === 'supervision' ? 'sup' : 'can'}-${row.requestId}-reject`
                                                                    ? '…'
                                                                    : 'Reject'}
                                                            </button>
                                                        </div>
                    ...
                    <div ref={projectsRef} id="doctor-projects" style={S.card}>
                        ...
                        {projectsLoading ? (
                            ...
                            <div style={S.emptyState}>
                                ...
                                <p style={S.emptyDesc}>Projects you supervise will show here.</p>
                            </div>
                        ) : (
                            <div style={{ display: 'flex', flexDirection: 'column', gap: 10 }}>
                                    ...
                                    >
                                        <div style={{ minWidth: 0 }}>
                                            ...
                                            <p style={{ margin: 0, fontSize: 12, color: '#64748b' }}>
                                                {p.currentMembers} student{p.currentMembers !== 1 ? 's' : ''}
                                            </p>
                                        </div>
                                        <span
                                            style={{
...
const S: Record<string, CSSProperties> = {
    ...
    heroLeft: { flex: '1 1 280px', display: 'flex', flexDirection: 'column', alignItems: 'flex-start' },
    heroTitle: {
        fontSize: 28,
        fontWeight: 800,
        color: '#0f172a',
    ...
    heroSub: { fontSize: 14, color: '#64748b', margin: '0 0 8px', fontWeight: 500 },
    ...
    heroStats: { display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 10, flexShrink: 0, minWidth: 220 },
    statCard: {
        display: 'flex',
        alignItems: 'center',
        gap: 12,
    ...
    emptyDesc: { fontSize: 12, color: '#94a3b8', margin: 0, maxWidth: 280, lineHeight: 1.6 },
    acceptBtn: {
        padding: '7px 14px',
        borderRadius: 8,
        border: 'none',
