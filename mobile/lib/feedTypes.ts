...
export function isCompanyFeedItem(
  item: Pick<FeedItem, "sourceType" | "relatedEntityType">,
): boolean {
  return (
...
export function normalizeFeedItem(raw: Record<string, unknown>): FeedItem {
  ...
  return {
    ...
    sourceImageBase64: (raw.sourceImageBase64 ?? raw.publisherImageBase64) as
      | string
      | null
      | undefined,
    ...
    recruitmentCampaignId: numOrUndef(
      raw.recruitmentCampaignId ?? raw.RecruitmentCampaignId,
    ),
    ...
    organizationProfileId: numOrUndef(
      raw.organizationProfileId ?? raw.OrganizationProfileId,
    ),
