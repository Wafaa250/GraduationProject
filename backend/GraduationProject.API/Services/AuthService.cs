...
namespace GraduationProject.API.Services
{
	...
	{
		Task<(AuthResponseDto? result, string? error)> RegisterStudentAsync(RegisterStudentDto dto);
	...
	public class AuthService : IAuthService
	{
		...
		// REGISTER STUDENT
		// ===========================
		public async Task<(AuthResponseDto? result, string? error)> RegisterStudentAsync(RegisterStudentDto dto)
		{
			var emailCheck = await CheckEmailAsync(dto.Email);
			if (emailCheck != null) return (null, emailCheck);

			var user = CreateUser(dto.Name, dto.Email, dto.Password, "student");
			_db.Users.Add(user);
		...
		private AuthResponseDto BuildResponse(User user, int profileId)
		{
		...
		private async Task<int> GetProfileIdAsync(User user)
		{
			...
			{
				"student" => (await _db.StudentProfiles
					.FirstOrDefaultAsync(s => s.UserId == user.Id))?.Id ?? 0,

				"doctor" => (await _db.DoctorProfiles
					.FirstOrDefaultAsync(d => d.UserId == user.Id))?.Id ?? 0,

				"company" => (await _db.CompanyProfiles
					.FirstOrDefaultAsync(c => c.UserId == user.Id))?.Id ?? 0,

				"association" => (await _db.AssociationProfiles
					.FirstOrDefaultAsync(a => a.UserId == user.Id))?.Id ?? 0,
