using System.Text.Json;
using GraduationProject.API.Data;
using GraduationProject.API.Models;

namespace GraduationProject.API.Data.Seeding.Phases
{
    internal static class ExpansionAcademicPhase
    {
        public static async Task RunAsync(ApplicationDbContext db, ExpansionContext ctx)
        {
            await AddGraduationProjectsAsync(db, ctx);
            await AddCoursesAsync(db, ctx);
        }

        private static async Task AddGraduationProjectsAsync(ApplicationDbContext db, ExpansionContext ctx)
        {
            var toAdd = Math.Max(0, ExpansionCatalog.TargetGraduationProjects - ctx.GraduationProjects.Count);
            if (toAdd == 0) return;

            var owners = ctx.StudentsWithoutProjects();
            if (owners.Count < toAdd)
                owners = ctx.Students.OrderBy(_ => ctx.Rng.Next()).Take(toAdd).ToList();

            var projectDefs = ExpansionCatalog.AdditionalProjects
                .Concat(SeedCatalog.ProjectDefs)
                .Take(toAdd)
                .ToList();

            for (var i = 0; i < toAdd && i < owners.Count; i++)
            {
                var def = projectDefs[i % projectDefs.Count];
                var owner = owners[i];
                var createdAt = ctx.DaysAgo(ctx.Rng.Next(30, 500));

                var project = new StudentProject
                {
                    OwnerId = owner.Id,
                    Name = def.Name + (i >= ExpansionCatalog.AdditionalProjects.Length ? $" (Cohort {i / 10 + 2})" : ""),
                    Abstract = def.Abstract,
                    ProjectType = def.Type,
                    RequiredSkills = JsonSerializer.Serialize(def.Skills),
                    PartnersCount = def.Partners,
                    CreatedAt = createdAt,
                    UpdatedAt = createdAt.AddDays(ctx.Rng.Next(5, 40)),
                };
                db.StudentProjects.Add(project);
                await db.SaveChangesAsync();
                ctx.GraduationProjects.Add(project);
                ctx.ProjectOwnerIds.Add(owner.Id);
                ctx.Increment("graduation_projects");

                db.StudentProjectMembers.Add(new StudentProjectMember
                {
                    ProjectId = project.Id,
                    StudentId = owner.Id,
                    Role = "leader",
                    JoinedAt = createdAt,
                });
                ctx.Increment("graduation_project_members");

                var isComputerEngineering = string.Equals(owner.Major, "Computer Engineering", StringComparison.Ordinal);
                if (isComputerEngineering)
                    project.PartnersCount = 2;

                var teamSize = isComputerEngineering ? 2 : Math.Min(def.Partners, 4);
                var teammatePool = ctx.Students.Where(s => s.Id != owner.Id).ToList();
                if (isComputerEngineering)
                {
                    var cePool = teammatePool.Where(s => string.Equals(s.Major, "Computer Engineering", StringComparison.Ordinal)).ToList();
                    if (cePool.Count > 0) teammatePool = cePool;
                }
                var teammates = ctx.PickMany(teammatePool, teamSize - 1).ToList();

                foreach (var mate in teammates)
                {
                    db.StudentProjectMembers.Add(new StudentProjectMember
                    {
                        ProjectId = project.Id,
                        StudentId = mate.Id,
                        Role = "member",
                        JoinedAt = createdAt.AddDays(ctx.Rng.Next(2, 20)),
                    });
                    ctx.Increment("graduation_project_members");
                }

                var doctor = ctx.Doctors[i % ctx.Doctors.Count];
                var supStatus = i % 5;
                if (supStatus == 0)
                {
                    project.SupervisorId = doctor.Id;
                    db.SupervisorRequests.Add(new SupervisorRequest
                    {
                        ProjectId = project.Id,
                        DoctorId = doctor.Id,
                        SenderId = owner.Id,
                        Status = "accepted",
                        CreatedAt = createdAt.AddDays(7),
                        RespondedAt = createdAt.AddDays(10),
                    });
                }
                else if (supStatus is 1 or 2)
                {
                    db.SupervisorRequests.Add(new SupervisorRequest
                    {
                        ProjectId = project.Id,
                        DoctorId = doctor.Id,
                        SenderId = owner.Id,
                        Status = supStatus == 1 ? "pending" : "rejected",
                        CreatedAt = ctx.DaysAgo(ctx.Rng.Next(5, 30)),
                        RespondedAt = supStatus == 2 ? ctx.DaysAgo(ctx.Rng.Next(1, 4)) : null,
                    });
                }
                ctx.Increment("supervisor_requests");

                if (i % 2 == 0)
                {
                    var invitee = ctx.Students.First(s => s.Id != owner.Id && teammates.All(t => t.Id != s.Id));
                    var invStatus = (i % 8) switch
                    {
                        0 => "pending",
                        2 => "accepted",
                        4 => "rejected",
                        6 => "expired",
                        _ => "cancelled",
                    };
                    db.ProjectInvitations.Add(new ProjectInvitation
                    {
                        ProjectId = project.Id,
                        SenderId = owner.Id,
                        ReceiverId = invitee.Id,
                        Status = invStatus,
                        CreatedAt = ctx.DaysAgo(ctx.Rng.Next(3, 40)),
                        RespondedAt = invStatus == "pending" ? null : ctx.DaysAgo(ctx.Rng.Next(1, 10)),
                    });
                    ctx.Increment("project_invitations");
                }

                if (project.SupervisorId != null && i % 11 == 0)
                {
                    db.SupervisorCancellationRequests.Add(new SupervisorCancellationRequest
                    {
                        ProjectId = project.Id,
                        DoctorId = doctor.Id,
                        SenderId = owner.Id,
                        Status = "pending",
                        CreatedAt = ctx.DaysAgo(6),
                    });
                    ctx.Increment("supervisor_cancellation_requests");
                }
            }
            await db.SaveChangesAsync();
        }

        private static async Task AddCoursesAsync(ApplicationDbContext db, ExpansionContext ctx)
        {
            var existingCodes = ctx.Courses.Select(c => c.Code).ToHashSet(StringComparer.OrdinalIgnoreCase);
            var toAdd = Math.Max(0, ExpansionCatalog.TargetCourses - ctx.Courses.Count);

            foreach (var def in ExpansionCatalog.AdditionalCourses.Take(toAdd))
            {
                if (existingCodes.Contains(def.Code)) continue;
                existingCodes.Add(def.Code);

                var doctor = ctx.Doctors[ctx.Rng.Next(ctx.Doctors.Count)];
                var course = new Course
                {
                    Name = def.Name,
                    Code = def.Code,
                    Semester = def.Semester,
                    DoctorId = doctor.Id,
                    CreatedAt = ctx.DaysAgo(ctx.Rng.Next(120, 500)),
                };
                db.Courses.Add(course);
                await db.SaveChangesAsync();
                ctx.Courses.Add(course);
                ctx.Increment("courses");

                var sections = new List<CourseSection>();
                for (var s = 0; s < 3; s++)
                {
                    var section = new CourseSection
                    {
                        CourseId = course.Id,
                        Name = $"Section {(char)('A' + s)}",
                        Days = JsonSerializer.Serialize(s switch
                        {
                            0 => new[] { "Sunday", "Tuesday" },
                            1 => new[] { "Monday", "Wednesday" },
                            _ => new[] { "Thursday" },
                        }),
                        TimeFrom = s == 0 ? "09:00" : s == 1 ? "11:00" : "14:00",
                        TimeTo = s == 0 ? "10:30" : s == 1 ? "12:30" : "15:30",
                        Capacity = 40,
                    };
                    db.CourseSections.Add(section);
                    sections.Add(section);
                    ctx.Increment("course_sections");
                }
                await db.SaveChangesAsync();
                ctx.CourseSections.AddRange(sections);

                var enrolled = ctx.PickMany(ctx.Students, ctx.Rng.Next(18, 28)).ToList();
                foreach (var student in enrolled)
                {
                    var section = sections[student.Id % sections.Count];
                    db.SectionEnrollments.Add(new SectionEnrollment
                    {
                        CourseSectionId = section.Id,
                        StudentProfileId = student.Id,
                    });
                    ctx.Increment("section_enrollments");

                    for (var m = 0; m < ctx.Rng.Next(1, 4); m++)
                    {
                        db.SectionChatMessages.Add(new SectionChatMessage
                        {
                            CourseSectionId = section.Id,
                            SenderUserId = student.UserId,
                            Text = ctx.Pick(ExpansionCatalog.ExtraMessageTemplates),
                            SentAt = ctx.DaysAgo(ctx.Rng.Next(1, 60)),
                        });
                        ctx.Increment("section_chat_messages");
                    }
                }

                var project = new CourseProject
                {
                    CourseId = course.Id,
                    Title = $"{def.Name} Team Project",
                    Description = $"Applied team project for {def.Name} covering design, implementation, and technical presentation.",
                    TeamSize = 4,
                    ApplyToAllSections = true,
                    AllowCrossSectionTeams = ctx.Rng.Next(2) == 0,
                    AiMode = ctx.Rng.Next(2) == 0 ? "doctor" : "student",
                    CreatedAt = ctx.DaysAgo(ctx.Rng.Next(60, 180)),
                };
                db.CourseProjects.Add(project);
                await db.SaveChangesAsync();
                ctx.CourseProjects.Add(project);
                ctx.Increment("course_projects");

                for (var t = 0; t < 4; t++)
                {
                    var team = new CourseTeam
                    {
                        CourseProjectId = project.Id,
                        TeamIndex = t,
                        CreatedAt = project.CreatedAt.AddDays(t),
                    };
                    db.CourseTeams.Add(team);
                    await db.SaveChangesAsync();
                    ctx.CourseTeams.Add(team);
                    ctx.Increment("course_teams");

                    var members = ctx.PickMany(enrolled, 4).ToList();
                    foreach (var member in members)
                    {
                        db.CourseTeamMembers.Add(new CourseTeamMember
                        {
                            CourseTeamId = team.Id,
                            StudentProfileId = member.Id,
                            UserId = member.UserId,
                            MatchScore = 68 + ctx.Rng.Next(0, 30),
                        });
                        ctx.Increment("course_team_members");
                    }

                    var conversation = new Conversation
                    {
                        Title = $"{def.Code} Team {t + 1}",
                        CourseTeamId = team.Id,
                        CreatedAt = team.CreatedAt,
                    };
                    db.Conversations.Add(conversation);
                    await db.SaveChangesAsync();
                    ctx.Increment("conversations");

                    foreach (var member in members)
                    {
                        db.ConversationUsers.Add(new ConversationUser
                        {
                            ConversationId = conversation.Id,
                            UserId = member.UserId,
                        });
                        ctx.Increment("conversation_users");
                    }

                    for (var m = 0; m < 12; m++)
                    {
                        var sender = members[m % members.Count];
                        var sentAt = team.CreatedAt.AddHours(m * 5 + ctx.Rng.Next(1, 4));
                        db.CourseTeamMessages.Add(new CourseTeamMessage
                        {
                            CourseTeamId = team.Id,
                            SenderUserId = sender.UserId,
                            Text = ctx.Pick(ExpansionCatalog.ExtraMessageTemplates),
                            SentAt = sentAt,
                        });
                        ctx.Increment("course_team_messages");

                        db.Messages.Add(new Message
                        {
                            ConversationId = conversation.Id,
                            SenderId = sender.UserId,
                            Text = ctx.Pick(ExpansionCatalog.ExtraMessageTemplates),
                            CreatedAt = sentAt,
                            Seen = m < 10,
                        });
                        ctx.Increment("messages");
                    }
                }
            }
            await db.SaveChangesAsync();
        }
    }
}
